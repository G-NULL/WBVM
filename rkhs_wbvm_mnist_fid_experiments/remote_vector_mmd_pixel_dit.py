...
def start_run(client: paramiko.SSHClient, remote_dir: str) -> None:
    command = (
        f"cd {remote_dir}; "
        "nohup bash run_vector_mmd_pixel_dit.sh "
        "> outputs_mnist_route2_v3_pixel_dit_orchestrator.log 2>&1 < /dev/null & "
        "echo $!"
    )
    code, out, err = capture(client, command, timeout=60)
    if code != 0:
        raise RuntimeError(err or "failed to start v3 pixel-DIT route-two run")
    print(f"STARTED v3 pixel-DIT route-two run pid={out.strip()}", flush=True)


def status_command(remote_dir: str) -> str:
    return f"""
cd {remote_dir}
status=$(cat outputs_mnist_route2_v3_pixel_dit.exit 2>/dev/null || echo running)
alive=$(ps -eo cmd | grep -E '[m]nist_fid_experiment.py|[r]un_vector_mmd_pixel_dit.sh' | wc -l)
gpu=$(nvidia-smi --query-gpu=memory.used,utilization.gpu --format=csv,noheader | head -1 2>/dev/null || echo no-gpu)
log_bytes=$(stat -c %s outputs_mnist_route2_v3_pixel_dit.log 2>/dev/null || echo 0)
metrics=$(test -s outputs_mnist_route2_v3_pixel_dit/metrics_summary.csv && echo yes || echo no)
...
def wait_for_run(client: paramiko.SSHClient, args: argparse.Namespace) -> None:
    ...
    while True:
        if time.time() - start > args.max_wait_seconds:
            raise TimeoutError(f"remote v3 pixel-DIT route-two run exceeded {args.max_wait_seconds} seconds")
        ...
        if status == "0":
            return
        if status != "running":
            raise RuntimeError(f"remote v3 pixel-DIT route-two run failed: {line}")
        if alive == "0":
            raise RuntimeError(f"remote v3 pixel-DIT route-two run stopped before completion: {line}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run route-two vMMD-WBVM with v3 pixel-DIT settings")
    ...
    try:
        ...
        if not args.no_fetch:
            ...
            print("FETCHED v3 pixel-DIT route-two outputs", flush=True)
    finally:
        client.close()
